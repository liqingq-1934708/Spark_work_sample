 ...
 |-- div5tailnum: integer (nullable = true)
   */
    ...
    public static JavaRDD<Row> QA(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
        JavaRDD<Row> rdd = d.javaRDD();
        JavaRDD<Row> resultA = rdd.filter(r -> r.getString(ORIGIN_CITY_NAME).equals("Seattle, WA"))
                                  .map(r -> RowFactory.create(r.getString(DEST_CITY_NAME)))
                                  .distinct()
                                  .sortBy(t -> t.getString(0), true, 1);
        resultA.foreach(s -> System.out.println(s));
        return resultA;
    }
    ...
    public static JavaRDD<Row> QB(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
        Dataset<Row> result = d.where(d.col("cancelled").notEqual(1))
                .groupBy("origincityname", "month")
                .count();
        result.show();
        JavaRDD<Row> resultB = result.javaRDD();
        return resultB;
    }
    ...
    public static JavaRDD<Row> QC(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
        Dataset<Row> result = d.where(d.col("depdelay").isNotNull())
                               .groupBy("origincityname")
                               .avg("depdelay");
        result.show();
        JavaRDD<Row> resultC = result.javaRDD();
        return resultC;
    }
